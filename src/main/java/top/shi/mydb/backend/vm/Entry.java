package top.shi.mydb.backend.vm;
...
public class Entry {
    ...
    private static final int OF_XMIN = 0;//该条记录（版本）的事务编号，
    private static final int OF_XMAX = OF_XMIN+8;//删除该条记录（版本）的事务编号。
    private static final int OF_DATA = OF_XMAX+8;//这条记录持有的数据
    ...
    public void setXmax(long xid) {
        dataItem.before();//需要对 DataItem 执行 before()
        try {
            SubArray sa = dataItem.data();
